...
namespace irr {
	...
	// -----------------------------------------------------------------------------
        int ChIrrTools::drawAllLinkframes(chrono::ChSystem& mphysicalSystem, video::IVideoDriver* driver, double scale, const std::string& name) {
		driver->setTransform(video::ETS_WORLD, core::matrix4());
		video::SMaterial mattransp;
		mattransp.ZBuffer = true;
		mattransp.Lighting = false;
		...
		while (myiter.HasItem()) {
			if ((*myiter).IsType<chrono::ChLinkBase>()) {
				...
				if (name != ""){
                                        std::string linkname = mylinkbase->GetNameString();
                                        if (linkname != name)
					{
						++myiter;
						continue;
				...
				driver->draw3DLine(core::vector3dfCH(p0), core::vector3dfCH(pz), mcol);
                                ...
                                if (mylinkbase->GetNameString() == name){
					break;
				}
			}
